$(document).ready(function () {
    ...
    const endpoint = 'https://raw.githubusercontent.com/lukasd2/Drukarnia-Format/master/mergedDataset.json?token=ATGsMaJ5IMDnR3JDNNBCsmY6QjlFKby_ks5btUZnwA%3D%3D'
    ...
    function dataToCharts(data) {
        ...
        //sort data for each country income level group, 
        data.forEach(key => {
            if (key.incomeLevel == "HIC") {
                tot1 += Number(key.Tasso);
                countries.HIC.push(Number(key.Tasso));
                totalHIC++;
            } else if (key.incomeLevel == "UMC") {
                tot2 += Number(key.Tasso);
                countries.UMC.push(Number(key.Tasso));
                totalUMC++;
            } else if (key.incomeLevel == "LMC" || key.incomeLevel == "LIC") {
                tot3 += Number(key.Tasso);
                countries.LMC.push(Number(key.Tasso));
                totalLMC++;
            }
        });
        mediaHIC = Math.round(tot1 / totalHIC);
        mediaUMC = Math.round(tot2 / totalUMC);
        mediaLMC = Math.round(tot3 / totalLMC);
        generateCharts(data, totalHIC, totalUMC, totalLMC);
    };
    ...
    function generateCharts(data, totalHIC, totalUMC, totalLMC) {
        ...
        computeWealthCorrelation(year2015);
        tassoMedioFasceGraph(countries, mediaHIC, mediaUMC, mediaLMC);
        tassoMedioPercGraph(continents, mediaAP, mediaEU, mediaLA, mediaME, mediaNA);
    ...
    //Second Graph
    function tassoMedioFasceGraph(data, mediaHIC, mediaUMC, mediaLMC) {
        console.log("testing", mediaHIC);
        var opzioni = {
            chart: {
                type: 'spline'
            },
     ...
     //Third Graph - left
    function tassoMedioPercGraph(data, mediaAP, mediaEU, mediaLA, mediaME, mediaNA) {
        var opzioni = {
            chart: {
                type: 'bar',
                backgroundColor: 'rgba(0,0,0,0)'
