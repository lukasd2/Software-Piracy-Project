...
$(document).ready(function () {
    ...
    fetch(endpointAPICollationResult)
        .then((res) => res.json())
        .then((data) => {
            mergedDateForCharts.push(...data);
        })
        .catch((error) => console.error(error));

    const countriesDataByYear = [];
    const countriesFromDataset = [];
    const countriesName = [];

    fetch(endpointInitialDB)
        .then((res) => res.json())
        .then((data) => {
            countriesDataByYear.push(...data["softwareup"]);
            countriesFromDataset.push(...data["countries"]);
            countriesName.push(
                ...removeDuplicatesBy((x) => x.Codice, countriesDataByYear)
            ); //remove duplicates by Code property 464 / 4 = 116
            //Remove object proprieties by copying only relevant proprieties
            for (let i = 0; i < countriesName.length; i++) {
                countriesName[i] = {
                    Codice: countriesName[i].Codice,
                    Nazione: countriesName[i].Nazione,
                    longitude: countriesName[i].longitude,
                    latitude: countriesName[i].latitude,
                };
            }
            //map generator function
            mappy(countriesDataByYear);
            //chart generation functions
            generateCharts(countriesDataByYear, mergedDateForCharts);
        })
        .catch((error) => console.error(error));
            ...
            if (flag) {
                ...
                $("#description").append(
                    "<p class='first f32' style='font-weight:bold; font-size: 1.2em; margin-left: 20px;'>" +
                        ...
                        " " +
                        "<span class='flag " +
                        countriesName[i].Codice.toLowerCase() +
                        "'></span></p>"
                );
                $("#description").append(
                    "<p class='first' style='font-size:18px; margin-left: 20px;'>É possibile confrontare fino a cinque paesi, da notare " +
    ...
    function graphLimit() {
        $(".first").remove();
        $("#description").append(
            "<p class='first' style='font-weight:bold; margin-left: 20px;'>Limite raggiunto, ricarica il grafico per un nuovo confronto!</p>"
        );
    ...
    function computeWealthCorrelation(data) {
        ...
        generateWealthCorrelationGraph(
            countries.HIC,
            countries.UMC,
            countries.LMC,
            countries.name
    ...
    function generateWealthCorrelationGraph(HIC, UMC, LMC, name) {
        var opzioni = {
            ...
            title: {
                text:
                    "Correlazione tra il reddito per capita e il tasso totale del software illegale",
            },
            subtitle: {
                text: null,
            },
            ...
            plotOptions: {
                scatter: {
                    tooltip: {
                        ...
                        pointFormat:
                            "Tasso: {point.x} %,<br> Reddito: {point.y} $",
                    },
                },
            },
            series: [
    ...
    function tassoMedioPercGraph(mediaAP, mediaEU, mediaLA, mediaME, mediaNA) {
        var opzioni = {
            ...
            title: {
                text:
                    "Tasso medio percentuale del software illegale scaricato nel 2015 per area",
            },
            subtitle: {
                text: null,
            },
    ...
    function valoreTotaleGraph(apVal, euVal, laVal, meVal, naVal) {
        var opzioni = {
            ...
            title: {
                text:
                    "Valore totale in milioni del software illegale scaricato nel 2015 per area",
            },
            subtitle: {
                text: null,
            },
